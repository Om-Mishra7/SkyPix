...
class Image_Editor:
    ...
    def _watermark(self, text, font_size=12):
        ...
        font_path = "backend/fonts/roboto.ttf"  # You can adjust this path to another font if needed
    ...
    def get_image_bytes(self, format="PNG"):
        ...
        # Ensure the image has an alpha channel for transparency
        if self.image.mode != 'RGBA':
            self.image = self.image.convert('RGBA')  # Convert to RGBA if not already in that mode
        
        # Create a BytesIO object to save the image in memory
        img_byte_arr = BytesIO()
        self.image.save(img_byte_arr, format=format)  # Save the image in the specified format
